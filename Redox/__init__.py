...
def get_user_list(__init__, key):
    with open("{}/Redox/{}".format(os.getcwd(), __init__), "r") as json_file:
        return json.load(json_file)[key]


# enable logging
FORMAT = "[ʀᴇᴅᴏx] %(message)s"
logging.basicConfig(
...
LOGGER = logging.getLogger("[ʀᴇᴅᴏx]")
LOGGER.info("ʀᴇᴅᴏx ɪs sᴛᴀʀᴛɪɴɢ. | ᴀɴ ғᴇᴇʟɪɴɢsxᴅᴄʜᴀᴛ ᴘʀᴏᴊᴇᴄᴛ ᴘᴀʀᴛs. | ʟɪᴄᴇɴsᴇᴅ ᴜɴᴅᴇʀ ɢᴘʟᴠ3.")
...
SPAMWATCH_SUPPORT_CHAT = "@FeelingsXDChat"
...
try:
    ...
    raise Exception(
        "[ʀᴇᴅᴏx ᴇʀʀᴏʀ]: Your ʀᴇᴅɪs ᴅᴀᴛᴀʙᴀsᴇ ɪs ɴᴏᴛ ᴀʟɪᴠᴇ, ᴘʟᴇᴀsᴇ 𝐂𝐡𝐞𝐜𝐤 ᴀɢᴀɪɴ."
    )
finally:
    REDIS.ping()
...
telegraph = Telegraph()
telegraph.create_account(short_name="Redox")
    ...
    if not isinstance(entity, Chat):
        ...
        try:
            ...
            for Nihal in apps:
                if Nihal != client:
                    try:
                        entity = await Nihal.get_chat(entity)
                    except (PeerIdInvalid, ChannelInvalid):
                        pass
                    else:
                        entity_client = Nihal
...
from Redox.modules.helper_funcs.handlers import CustomMessageHandler, CustomRegexHandler

tg.RegexHandler = CustomRegexHandler
tg.MessageHandler = CustomMessageHandler

from Redox.modules.helper_funcs.handlers import CustomCommandHandler
...
try:
    from Redox.antispam import antispam_cek_user, antispam_restrict_user, detect_user
